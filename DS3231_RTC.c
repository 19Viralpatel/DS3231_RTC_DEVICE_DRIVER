...
static struct task_struct *alarm1_thread;
int alarm1_thread_function(void *pv);
...
static int I2C_Write(unsigned char *buf, unsigned int len)
{
   ...
   // pr_info("In I2C_WRITE : Sends data to an I2C device ");
    return ret;
}
...
static int I2C_Read(unsigned char *out_buf, unsigned int len)
{
   ...
   // pr_info("In I2C_READ : Read  data from an I2C device ");
    return ret;
}
...
static int __init ds3231_init(void)
{
    ...
    if (rtc_i2c_adapter != NULL) {
    	...
    	alarm1_thread = kthread_create(alarm1_thread_function,NULL,"alarm1 Thread");
        if(alarm1_thread) {
            wake_up_process(alarm1_thread);
        } else {
            pr_err("Cannot create kthread\n");
        }
#if 0
        /* You can use this method also to create and run the thread */
        alarm1_thread = kthread_run(alarm1_thread_function,NULL,"alarm1 Thread");
        if(alarm1_thread) {
    ...
    pr_info("Init Over");
    pr_info("DS3231 Driver Added!!! ret : %d\n", ret);
